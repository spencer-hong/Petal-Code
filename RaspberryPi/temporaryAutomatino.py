...
class Profile(object):
    """Instances represent environment control profiles.
    Instance Attributes:
        name: name of the profile [String]
        temperatureSP: temperature set-point to maintain [int in 0..100]
        humiditySP: humidity set-point to maintain [int in 0..100]
        lightSP: light set-point in % of duty cycle [int in 0..100]
        blueSP: blue light set-point in % of duty cycle [int in 0..100]"""

    def __init__(self, name, temperatureSP, humiditySP, lightSP, blueSP):
        assert type(name) == str
        assert type(temperatureSP) == int
        assert type(humiditySP) == int
        assert type(lightSP) == int
        assert type(blueSP) == int
        assert 0 <= temperatureSP and temperatureSP <= 100
        assert 0 <= humiditySP and humiditySP <= 100
        assert 0 <= lightSP and lightSP <= 100
        assert 0 <= blueSP and blueSP <= 100
        self.name = name
    ...
    def getTemperatureSP(self):
        """Returns: temperatureSP attribute"""
        return self.temperatureSP

    def getHumiditySP(self):
        """Returns: humiditySP attribute"""
        return self.humiditySP

    def getLightSP(self):
        """Returns: lightSP attribute"""
        return self.lightSP

    def getBlueSP(self):
        """Returns: blueSP attribute"""
        return self.blueSP

    def changeProfile(self, name, temperatureSP, humiditySP, lightSP, blueSP):
        """Change the instance attributes of the profile"""
        assert type(name) == str
        assert type(temperatureSP) == int
        assert type(humiditySP) == int
        assert type(lightSP) == int
    ...
    def __str__(self):
        ...
        return ("{" + self._name + "; temp: " + str(self._temperatureSP) + " C; humidity: " + str(self._humiditySP) + "%; light: " + str(self._lightSP) + "%; blue: " + str(self._blueSP) + "%}" )

day = Profile("day", 24, 50, 100, 0)
night = Profile("night", 19, 50, 0, 0)
germ = Profile("germ", 24, 50, 0, 100)

profiles = [day, night, germ]
...
def add_profile(profile):
    """Adds a new profile to profiles or updates an existing one if a profile with
    the same NAME already exists
    PRECONDITIONS:
        profile is of type Profile
    """
    try:
        # assert isinstance(profile, Profile)
        global profiles
        indices = [profiles.index(x) for x in profiles if x.getName() == profile["name"]]
        if len(indices) == 0:
            print("adding new profile \"" + "\": " + str(profile))
            p = Profile(profile["name"], profile["temperatureSP"], profile["humiditySP"], profile["lightSP"], profile["blueSP"])
            profiles.append(p)
        else:
            print("updating existing profile \"" + "\": " + str(profile))
            profiles[indices[0]].changeProfile(profile["name"], profile["temperatureSP"], profile["humiditySP"], profile["lightSP"], profile["blueSP"])
...
def update_current_profile(arg = False):
    ...
    if len(profiles) == 1:
        ...
        if maximin is None:
            maximin = last
        indices = [profiles.index(x) for x in profiles if x.getName() == schedule[maximin][0]]
        if len(indices) >= 1:
            if not current_profile.getName() == (profiles[indices[0]].getName()):
...
def automate():
    ...
    tempSP = current_profile.getTemperatureSP()
    ...
    if (light_override):
        ...
        analog_write(light_pwm, current_profile.getLightSP())
    analog_write(blue_pwm, current_profile.getBlueSP())
    blue = current_profile.getBlueSP()
    light = current_profile.getLightSP()
...
def water_add():
    waterAddAnalog = 0;
    waterAddDigital = GPIO.input(ADDING_WATER_PIN);
    ...
    if connflag == True:
        if humreading is not None and tempreading is not None:
            mqttc.publish("$aws/things/" + thingName + "/shadow/update",
                ...
                ", \"tempSP\":" + str(current_profile.getTemperatureSP()) + 
