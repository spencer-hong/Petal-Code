...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe(thingName + "/change_light", 1)
    ...
    if (val >= 0) and (val <= 100):
        if val != light:
            ...
            mqttc.publish("$aws/things/" + thingName + "/shadow/update",
                "{\"state\":{\"desired\":{\"light\": null" + "}}}", qos=1)
    else:
...
def resolve_deltas(json):
    ...
    if 'desired' in json["state"]:
        ...
        if 'light' in json["state"]["desired"]:
            val = json["state"]["desired"]["light"]
            change_light(val)
